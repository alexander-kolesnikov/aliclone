...
20.times do |i|
  ...
  rand(10).times do |c|
    ...
    Comment.create(author_id: @user, contents: Faker::Lorem.sentences(5), commentable: vendor)
  end
  rand(50).times do |j|
    @random_category=Category.order('RANDOM()').first
    ...
    rand(10).times do |c|
      @user = User.order('RANDOM()').first
      Comment.create(@user, contents: Faker::Lorem.sentences(5), commentable: product)
    end
    (1+rand(5)).times do |k|
...
2000.times do |c|
  ...
  @user = User.order('RANDOM()').first
  Comment(author_id: @user, contents: Faker::Lorem.sentences(5), commentable: @comment)
endRAILS_ENV=development